...
- **Status:** RESOLVED 2026-07-14 (branch `feat/trust-status-generator`). `python -m ops.cli trust status` now exists: milestone rows are declared once in `ops/trust_release_status.py` (MILESTONES) with explicit `manual` vs `derived` provenance; derived rows are corroborated by local read-only checks (repo files, symbols, `REPORT_SCHEMA_VERSION==2`, M7 schema gate); hermes ledger-audit operatorSummary is cited read-only (latest under `ai-logs/hermes/runs/` or `--evidence <json>`); gh run-conclusion checks only behind `--live-gh` (offline by default); `--out <file>` writes UTF-8. Strategy-doc table + instruction regenerated. Remaining follow-on: wiki hot-cache pointer refresh (vault lives outside the repo) and any M7 max-age/expiry semantics.
