...
- **Current context:** see `@docs/claude/active-sprint.md` for the refreshed Hermes Track A completion handoff, rebuilt on 2026-06-02 from live `main` / `origin/main` at `ae5c573`.
- **Hermes Track A:** H0-H5 is merged through PR #260 (`feat: add Hermes ledger audit rehearsals`). The completed post-PR235 sequence covers plan contracts, deliberation quorum/readiness, ledger-audit v2 subsystems, bypass lifecycle audit, operator summaries, lock-order assertions, failure-event artifacts, CI/nightly audit behavior, and cross-task rehearsals. No GitHub PRs were open at the last live refresh. Treat older PR #191/#192 and PR #235-era handoff notes as historical, not current active work.
...
- **Hermes code work:** no next Hermes code slice is named by current post-PR260 evidence. Future Hermes code changes must begin from fresh live discovery of the registry, CLI, emitted artifacts, docs/spec state, and PR state before choosing a worktree slice.
...
# Quality ops
python -m ops.cli quality stats --db "$DISCOVERY_DB_PATH" --days 30
