...
class MemoryExtractor:
    ...
    def _get_llm_client(self):
        ...
        try:
            ...
            # timeout is in milliseconds (int) per official docs
            http_options = HttpOptions(timeout=60000)  # 60s
    ...
    def _normalize_fact_content(text: Optional[str]) -> str:
        """Normalize fact content with null-safety."""
    ...
    def run(self, max_items: int = 10):
        ...
        try:
            for decision in decisions:
                ...
                # Transaction is atomic due to fixed storage.transaction()
                with self.storage.transaction() as conn:
                    if is_suspicious:
                        conn.execute(
                            """
